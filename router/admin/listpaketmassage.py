...
app = APIRouter(prefix=("/listmassage"))

@app.get('/getdatapaketmassage')
async def getdatapaketmassage() :
  try :
    pool = await get_db()
    ...
    async with pool.acquire() as conn:
      async with conn.cursor() as cursor:
        ...
        q1 = "SELECT * FROM paket_massage ORDER BY id_paket_msg ASC"
...
@app.put('/updatepaketmassage')
async def updatepaketmassage(
  request: Request
):
  try:
    pool = await get_db()
...
@app.delete('/deletepaketmassage')
async def deletepaketmassage(
  request: Request
):
  try:
    pool = await get_db()
    ...
    async with pool.acquire() as conn:
      async with conn.cursor() as cursor:
        try:
          ...
          # 2. Execute querynya
          data = await request.json()
          q1 = "DELETE FROM paket_massage WHERE id_paket_msg = %s"
          await cursor.execute(q1, (data['id_paket_msg']))
          # 3. Klo Sukses, dia bkl save ke db
          await conn.commit()
