    ...
    async with pool.acquire() as conn:
      async with conn.cursor() as cursor:
        try:
          ...
          q2 = "INSERT INTO detail_promo_kunjungan(detail_kode_promo,limit_kunjungan,durasi,discount,harga_promo) VALUES(%s,%s,%s,%s,%s)"
          await cursor.execute(q2, (lastiddetailpromo,data['limit_kunjungan'],data['durasi'],data['discount'],data['harga_promo']))
          # 3. Klo Sukses, dia bkl save ke db
          await conn.commit()
